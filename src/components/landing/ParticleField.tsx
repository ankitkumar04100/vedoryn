...
function FloatingOrb({ position, color, scale, speed, index }: { position: [number, number, number]; color: string; scale: number; speed: number; index: number }) {
  ...
  useFrame((state) => {
    ...
    ref.current.position.x = position[0] + Math.cos(t * 0.7 + index) * 0.3;
  });
