...
import { Crown, Mail, Lock, User, ArrowRight, Sparkles } from "lucide-react";
...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        toast.success("Welcome back to your royal dashboard!");
        navigate("/dashboard");
      } else {
  ...
  const handleGuest = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Welcome, Guest! Explore the royal platform freely.");
      navigate("/dashboard");
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-accent/8 rounded-full blur-3xl animate-float" style={{ animationDelay: "2s" }} />
      </div>
      ...
      >
        <div className="text-center mb-8">
          <motion.div whileHover={{ rotate: 360 }} transition={{ duration: 0.6 }} className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-royal mb-4 shadow-royal">
          ...
          <p className="text-muted-foreground mt-2 text-sm">
            {isLogin ? "Sign in to your royal career dashboard" : "Start your AI-powered career journey"}
          </p>
        </div>

        <div className="glass rounded-2xl p-8 shadow-elevated royal-border">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full bg-gradient-royal text-primary-foreground font-semibold hover:opacity-90 shadow-glow">
          ...
          <div className="space-y-3">
            <Button variant="outline" onClick={handleGoogle} className="w-full font-medium royal-border">
