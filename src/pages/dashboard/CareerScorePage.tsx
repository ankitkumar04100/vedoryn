...
import { Brain, TrendingUp, Code, Briefcase, Mic, BookOpen, Loader2 } from "lucide-react";
...
export default function CareerScorePage() {
  ...
  useEffect(() => {
    // Dynamic breakdown based on profile
    ...
    setBreakdown([
      ...
      { label: "Project Quality", value: Math.round(baseLevel * 0.8), icon: Briefcase, color: "bg-vedoryn-cyan" },
      { label: "Interview Readiness", value: Math.round(baseLevel * 0.7), icon: Mic, color: "bg-vedoryn-pink" },
      { label: "Consistency", value: Math.min((profile?.daily_hours ?? 2) * 15, 100), icon: TrendingUp, color: "bg-vedoryn-green" },
  ...
  return (
    <div className="space-y-8 max-w-6xl">
      <div>
        <h1 className="font-display text-3xl font-bold flex items-center gap-3"><Brain className="w-8 h-8 text-primary" /> Career Score</h1>
